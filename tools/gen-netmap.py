...
SPECIAL = ("attacker", "firewall", "secutils", "kali")
...
def config(nn):
    ...
    if r.returncode != 0:
        raise SystemExit(f"compose config failed: {r.stderr[:300]}")
...
def build(nn):
    ...
    with open(f"{OUTDIR}/network.svg", "w") as f:
        f.write(svg)
    print(f"  network.svg  ({len(svcs)} machines, {len(nets)} network(s))")
